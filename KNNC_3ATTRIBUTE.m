...
knn1=fitcknn(traindata(:,[1 2 3]),trainclass,'classNames',{'setosa','versicolor','virginica'});
tic
predclass1=predict(knn1,testdata(:,[1 2 3]));
toc
...
subplot(4,2,2);
grid on;
gscatter3(att1(:,1),att1(:,2),att1(:,3),predclass1);
if flag==1
line(att1(diff,1),att1(diff,2),att1(diff,3),'marker','o','markersize',10,'linestyle','none');
end
xlabel 'SEPAL LENGTH (cm)';
ylabel 'SEPAL WIDTH (cm)';
zlabel 'PETAL LENGTH (cm)';
...
subplot(4,2,4);
grid on;
gscatter3(att1(:,2),att1(:,3),att1(:,4),predclass1);
if flag==1
line(att1(diff,2),att1(diff,3),att1(diff,4),'marker','o','markersize',10,'linestyle','none');
end
xlabel 'SEPAL WIDTH (cm)';
ylabel 'PETAL LENGTH (cm)';
zlabel 'PETAL WIDTH (cm)';
...
subplot(4,2,6);
grid on;
gscatter3(att1(:,1),att1(:,3),att1(:,4),predclass1);
if flag==1
line(att1(diff,1),att1(diff,3),att1(diff,4),'marker','o','markersize',10,'linestyle','none');
end
xlabel 'SEPAL LENGTH (cm)';
ylabel 'PETAL LENGTH (cm)';
zlabel 'PETAL WIDTH (cm)';
...
subplot(4,2,8);
grid on;
gscatter3(att1(:,1),att1(:,2),att1(:,4),predclass1);
if flag==1
line(att1(diff,1),att1(diff,2),att1(diff,4),'marker','o','markersize',10,'linestyle','none');
end
xlabel 'SEPAL LENGTH (cm)';
ylabel 'SEPAL WIDTH (cm)';
zlabel 'PETAL WIDTH (cm)';
...
att2=testdata;
knn2=fitcknn(traindata(:,[2 3 4]),trainclass,'classNames',{'setosa','versicolor','virginica'});
tic
predclass2=predict(knn2,testdata(:,[2 3 4]));
toc
...
att3=testdata;
knn3=fitcknn(traindata(:,[1 2 4]),trainclass,'classNames',{'setosa','versicolor','virginica'});
tic
predclass3=predict(knn3,att3(:,[1 2 4]));
toc
...
att4=testdata;
knn4=fitcknn(traindata(:,[1 3 4]),trainclass,'classNames',{'setosa','versicolor','virginica'});
tic
predclass4=predict(knn4,att4(:,[1 3 4]));
toc
%calculating indices of misclassified points
f=1;
flag=0;
