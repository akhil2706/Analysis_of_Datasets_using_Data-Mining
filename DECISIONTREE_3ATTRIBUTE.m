...
dt1=fitctree(traindata(:,[1 2 3]),trainclass,'classNames',{'setosa','versicolor','virginica'});
tic
predclass1=predict(dt1,testdata(:,[1 2 3]));
toc
...
att1=testdata;
%calculating loss
treeloss=(size(diff1)./size(testclass)).*100;
str11='The loss percentage is: ';
str01=num2str(treeloss(1,1));
str21=' %';
...
if flag==1
line(att1(diff1,2),att1(diff1,3),att1(diff1,4),'marker','o','markersize',10,'linestyle','none');
end
xlabel 'SEPAL WIDTH (cm)';
ylabel 'PETAL LENGTH (cm)';
zlabel 'PETAL WIDTH (cm)';
...
att2=testdata;
dt2=fitctree(traindata(:,[2 3 4]),trainclass,'classNames',{'setosa','versicolor','virginica'});
tic
predclass2=predict(dt2,testdata(:,[2 3 4]));
toc
%calculating indices of misclassified points
flag=0;
j=1;
...
%calculating loss
treeloss=(size(diff2)./size(testclass)).*100;
str12='The loss percentage is: ';
str02=num2str(treeloss(1,1));
str22=' %';
...
att3=testdata;
dt3=fitctree(traindata(:,[1 2 4]),trainclass,'classNames',{'setosa','versicolor','virginica'});
tic
predclass3=predict(dt3,att3(:,[1 2 4]));
toc
%calculating indices of misclassified points
flag=0;
f=1;
...
att4=testdata;
dt4=fitctree(traindata(:,[1 3 4]),trainclass,'classNames',{'setosa','versicolor','virginica'});
tic
predclass4=predict(dt4,att4(:,[1 3 4]));
toc
%calculating indices of misclassified points
flag=0;
f=1;
...
%calculating loss
treeloss=(size(diff4)./size(testclass)).*100;
str14='The loss percentage is: ';
str04=num2str(treeloss(1,1));
str24=' %';
